...
def verify_image(image_file):
    ...
    if image is None:
        ...
        raise AssertionError(
            f"remove the image and json file: {os.fsdecode(image_file)}"
        )
...
if __name__ == "__main__":
    ...
    for json_file, image_file in tqdm(
        ...
        if json_file.stem != image_file.stem:
            raise Exception(
                f"we have a big problem! meta and image file mismatch: {json_file.stem}.json not paired with {image_file.stem}.png"
            )

    n_jobs = os.cpu_count() if not os.environ.get("PRE_DEBUG", False) else 1
    Parallel(n_jobs=n_jobs)(
        delayed(verify_image)(image_file) for image_file in tqdm(image_files)
    )
    ...
    for sample_idx in sample_indices_val:
        ...
        if json_file.stem != image_file.stem:
            raise Exception(
                f"we have a big problem! meta and image file mismatch: {json_file.stem} != {image_file.stem}"
            )
        shutil.move(
            image_file,
            image_file.parent / "val" / f"{image_file.stem}{image_file.suffix}",
        )
        shutil.move(
            json_file, json_file.parent / "val" / f"{json_file.stem}{json_file.suffix}"
        )
    ...
    for sample_idx in sample_indices_test:
        ...
        shutil.move(
            image_file,
            image_file.parent / "test" / f"{image_file.stem}{image_file.suffix}",
        )
        shutil.move(
            json_file, json_file.parent / "test" / f"{json_file.stem}{json_file.suffix}"
        )
