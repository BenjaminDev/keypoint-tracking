...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Show mlcore model predictions")
    parser.add_argument(
        "--mpath", dest="model_path", type=Path, help="Path to the mlcore model",
    )
    parser.add_argument(
        "--ddir", dest="data_dir", type=Path, help="Path to the .png files",
    )
    parser.add_argument(
        "--mtype",
        dest="model_type",
    ...
    if args.model_type == "heatmap":
        ...
        for fn in tqdm(image_files):
            ...
            heatmap = Image.fromarray(
                np.uint8(cm.viridis(heatmaps.max(axis=0)[0]) * 255)
            )
            # breakpoint()
            # kps=outputs["coordinates"]
        ...
        for fn in tqdm(image_files):
            ...
            with torch.no_grad():
                heatmaps = m(y_hat)
            heatmap = Image.fromarray(
                np.uint8(cm.viridis(y_hat[:12].max(axis=0)[0]) * 255)
            )
            heatmap.save(
                f"/Volumes/external/wf/data/ARSession/out/{i}_tmp_heat_ref.png"
            )
            manual_decoded_keypoints = []
            for j in range(12):  # range(y_hat.shape[0]):
                manual_decoded_keypoints.append(
                    (y_hat[j] == torch.max(y_hat[j])).nonzero()[0].tolist()
                )
            annotated_image = draw_keypoints(
                image,
                manual_decoded_keypoints,
            ...
            annotated_image.save(
                f"/Volumes/external/wf/data/ARSession/out_ref/{i}_tmp.png"
            )
        ...
        for i, fn in tqdm(enumerate(image_files)):
            ...
            for bbox in outputs["coordinates"]:
                ...
                bboxes.append(
                    [
                        int(x * w) + int(w * relw) / 2,
                        int(y * h) + int(h * relh) / 2,
                        int(x * w) - int(w * relw) / 2,
                        int(y * h) - int(h * relh) / 2,
                    ]
                )
            annotated_image = draw_bounding_boxes(image, torch.tensor(bboxes))
            PIL.Image.fromarray(annotated_image.permute(1, 2, 0).numpy()).save(
                f"/Volumes/external/wf/data/ARSession/out_yolov5/{i}_tmp.png"
            )
    ...
    out = check_call(
        [
            "ffmpeg -framerate 25 -i  /Volumes/external/wf/data/ARSession/out_yolov5/%d_tmp.png -c:v libx264 -pix_fmt yuv420p /Volumes/external/wf/data/ARSession/out_yolov5/out_new.mp4"
        ],
        shell=True,
    )
